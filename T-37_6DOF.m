...
params.CLalpha = TBD; %per radian
params.CL0 = TBD ;
params.CLdelta_e = TBD; % per radian
params.CD0 = TBD;
params.weight = params.mass*params.g;
params.Cm0 = TBD;%.6;
params.CmAlpha = TBD; %per radian
params.CmDelta_e = TBD; % per radian
params.Cmq = TBD;
params.CmAlphaDot = TBD;
params.phi_T = TBD;% thrust vector pitch angle in deg.
params.dihedral = TBD*pi/180; %dihedral in radians
params.sweep = TBD*pi/180; %wing sweep in radians
params.Cy0 = TBD;
params.CyBeta = TBD; %per radian
params.CydeltaR = TBD; %per radian
params.Croll0 = TBD;
params.Cyaw0 = TBD;
params.CrollBeta = TBD;
params.CrollP = TBD;%*(params.wingSpan/(2*params.SpdCmd));
params.CrollR = TBD;%*(params.wingSpan/(2*params.SpdCmd));
params.CrollDeltaA = TBD;
params.CrollDeltaR = TBD;
params.CnBeta = TBD;
params.CnDeltaA = TBD;
params.CnDeltaR = TBD;
params.CnR = TBD;%*(params.wingSpan/(2*params.SpdCmd));
params.CnP = TBD;%*(params.wingSpan/(2*params.SpdCmd));
controls.delta_e_gain1 = -2e-04;%-2e-04;
controls.delta_e_gain2 = -1e-03;%-1e-02;
